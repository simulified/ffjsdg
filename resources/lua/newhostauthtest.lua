...
Players.PlayerRemoving:connect(function(Player)
	if funnyman[Player.Name] then
		funnyman[Player.Name] = nil
	end
end)
...
NetworkServer.ChildAdded:connect(function(child)
	...
	child:SetBasicFilteringEnabled(true)
	...
	if(child:GetPlayer()) then
		...
		child.NewFilter = function(item)
			local success, res = pcall(function()
				if item and item.Name == "token" then
		...
		child.DeleteFilter = function(item)										
			return Enum.FilterResult.Rejected
		end
		child.PropertyFilter = function(item, prop)
			local success, err = pcall(function()
				local plr = child:GetPlayer()
				if item:IsA("Player") and prop:lower() == "userid" and item == plr then
					if funnyman[plr.Name] == true then 
						return Enum.FilterResult.Rejected
					else
						funnyman[plr.Name] = true --this will break when we eventually release username changes (in march)
						return Enum.FilterResult.Accepted
					end
				end
				if item:IsA("Tool") then return Enum.FilterResult.Accepted end
				if item and item:IsA("Humanoid") then if game.Players:GetPlayerFromCharacter(item.Parent) == rep:GetPlayer() then return Enum.FilterResult.Accepted end end
				return Enum.FilterResult.Rejected
			end)
			if success then
				return res
		...
		child.EventFilter = function(item)		
