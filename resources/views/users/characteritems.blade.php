...
@section('meta')
<meta property="og:title" content="Character - {{ config('app.name') }}">
			...
			<div class="col-md-3">
				<div class="card shadow-sm">
					...
					<div class="card-body text-center">
						<button id="regenerateThumbnail" class="btn btn-primary btn-block shadow-sm">Regenerate</button>
				...
				<div class="card shadow-sm">
					<div class="card-header">
						<ul class="nav nav-tabs card-header-tabs">
                            ...
                            <li class="nav-item"><a class="nav-link{!! ($type == "Gear" ? ' active' : '') !!}" href="{{ route('users.characteritems', 'gears') }}">Gears</a></li>
						</ul>
					</div>
					<div class="card-body">
                        @if ($items->count() > 0)
                                        ...
                                        <div class="card mb-2 border-0 text-center" style="width: 100%; height: 175px">
                                            ...
                                            <div class="card-body p-1" style="display: block;">
                                                ...
                                                <form method="POST" action="{{ url('/character/toggle/' . $item->id) }}">
                                                    @csrf
                                                    <button type="submit" style="width: 100%" class="btn btn-sm btn-{{ ($item->wearing) ? "danger" : "primary" }} shadow-sm">{{ ($item->unequippable) ? "nope" : (($item->wearing) ? "Remove" : "Equip") }}</button>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
	...
	<script>
        ...
        $(document).ready(function() {
            ...
            setInterval(function() {
                console.log("Yooo");
                $('#userThumbnail').attr("src", "/users/1/thumbnail")
            }, 1000);
                ...
                $.ajax({
                    ...
                    success: function (msg){
                        if (msg.startsWith("OK")) {
                            ...
                            $('#userThumbnail').attr("src", "{{ asset('images/thumbnail/blank.png') }}");
                        }
                    }
                });
